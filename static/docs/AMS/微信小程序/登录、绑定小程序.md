...
|参数名|是否必选|类型|说明|
|:----    |:---|:----- |-----   |
|type |必须  |string |登录账号类型，type=account |
|username |必选  |string |用户名 |
|password |必选  |string |密码 |
|grant_type |必选  |string |授权类型:password,refresh_token,captcha |
|scope |必须  |string |固定all |
|code |必须  |string |微信小程序登录时获取的 code |
